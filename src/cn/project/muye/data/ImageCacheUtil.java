...
import cn.project.muye.utils.MD5;
...
public class ImageCacheUtil implements ImageCache {
    ...
    public ImageCacheUtil() {
        ...
        try {
            // 获取图片缓存路径
            File cacheDir = getDiskCacheDir(MuYeApplication.applicationContext, "thumb");
            if (!cacheDir.exists()) {
                cacheDir.mkdirs();
            }
            // 创建DiskLruCache实例，初始化缓存数据
            mDiskLruCache = DiskLruCache
                    .open(cacheDir, getAppVersion(MuYeApplication.applicationContext),
                            1, DISKMAXSIZE);
        } catch (IOException e) {
            e.printStackTrace();
        }
